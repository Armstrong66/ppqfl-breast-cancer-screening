...
Outputs (/kaggle/working/uq_outputs/):
...
Prerequisites: 3_5_vqc.py must have been run (Regime A checkpoint needed)
               2_baseline.py checkpoint needed for MC-Dropout
=============================================================================
"""
...
import pandas as pd
import matplotlib
matplotlib.use('Agg')  # headless rendering — required on SSH/HPC/cluster
...
VQC_CKPT_DIR = BASE / "vqc_outputs"        # base dir — subdirs appended per regime
CNN_CKPT_DIR = BASE / "baseline_outputs"

# ── Match these to what you trained in 2_baseline.py / 3_5_vqc.py ───
BACKBONE     = "mobilenetv2"
#VQC_N_QUBITS = 4          # best Regime A qubit count from your sweep
#VQC_N_LAYERS = 2
#VQC_LR       = 0.01       # used to locate the correct checkpoint filename

def auto_detect_best_vqc_config(ckpt_dir):
    """
    Scans the checkpoint directory for VQC checkpoints, cross-references
    their corresponding history files to find the configuration with the 
    highest validation AUC, and returns (n_qubits, n_layers, lr).
    """
    ckpt_dir = Path(ckpt_dir)
    all_ckpts = list(ckpt_dir.glob("vqc_q*.pt"))
    
    if not all_ckpts:
        raise FileNotFoundError(f"No VQC checkpoints (.pt) found in directory: {ckpt_dir}")

    best_auc = -1.0
    best_config = None

    for ckpt_path in all_ckpts:
        # Call .name on individual Path elements inside the loop
        ...
        if match:
            ...
            # Cross-reference with the history file generated by 3_5_vqc.py
            ...
            # Fallback assignment if history records aren't accessible
            if best_config is None:
                best_config = (n_qubits, n_layers, lr)
                print(f"Fellback on {best_config}. Still couldn't find the right/actual configs from 3_5_vqc")

    print(f"--> Auto-detected best VQC config: qubits={best_config}, layers={best_config}, lr={best_config} (Val AUC: {best_auc:.4f})")
    return best_config
